...
import { generateEmailCampaignContentInputSchema, generateEmailCampaignContentOutputSchema } from '@/lib/schemas/email-campaign-generator';
...
export async function sendEmailCampaign(data: z.infer<typeof emailCampaignSchema>) {
    ...
    // currently 'subscribers' is treated same as 'customers' or 'all' in this simplified version
    // In a real app, we would check mailing list subscription status.
    ...
    // Note: sendBulkEmail handles the actual sending logic (e.g. via Postmark)
    await sendBulkEmail(
        recipients,
...
export async function generateEmailCampaignContent({ productName, discountCode, audience }: { productName: string, discountCode?: string, audience: string }) {
    const prompt = ai.definePrompt({
        ...
        input: { schema: generateEmailCampaignContentInputSchema },
        output: { schema: generateEmailCampaignContentOutputSchema },
